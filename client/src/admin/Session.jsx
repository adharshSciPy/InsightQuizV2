...
function Session() {
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`http://13.50.6.227:8000/api/v1/section/deletesection/${sessionToDelete}`);
  ...
  const handleStartSession = async () => {
    try {
      ...
      await axios.patch("http://13.50.6.227:8000/api/v1/section/startquiz", {
        sectionId: sessionToStart,
        questionType: session?.questionType,
      });
  
      // Step 2: Update on the server with `togglestatus`
      await axios.patch("http://13.50.6.227:8000/api/v1/section/togglestatus", {
        sectionId: sessionToStart,
      });
  
      // Step 3: Call `checkactivebadge` and log the response
      const { data } = await axios.get("http://13.50.6.227:8000/api/v1/section/checkactivebadge", {
  ...
  const handleEndSession = async (sessionId) => {
    try {
      ...
      const { data } = await axios.patch("http://13.50.6.227:8000/api/v1/section/togglereverse", {
        sectionId: sessionId,
      });
  ...
  const getSessions = async () => {
    try {
      const res = await axios.get("http://13.50.6.227:8000/api/v1/section/getsections");
