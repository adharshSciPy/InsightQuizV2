...
function UserMcqTable() {
    ...
    const fetchSectionData = async () => {
        try {
            ...
            const response = await axios.get(`http://13.50.6.227:8000/api/v1/user/getusermcqperfomance/${userId}/${sessionId}`);
            ...
            const questionPromises = questionDetails.map(async (item) => {
                const questionResponse = await axios.get(
                    `http://13.50.6.227:8000/api/v1/user/getsinglemcqquestion/${sectionDetails.sectionId}`,
    ...
    const fetchSectionName=async()=>{
        try {
            const response=await axios.get(`http://13.50.6.227:8000/api/v1/section/getsectionsbyid/${sectionDetails.sectionId}`)
