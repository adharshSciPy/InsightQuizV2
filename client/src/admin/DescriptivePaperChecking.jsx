...
function DescriptivePaperChecking() {
  ...
  const fetchDescriptiveData = async () => {
    try {
      const questionResponse = await axios.get(
        `http://13.51.112.252:8000/api/v1/user/getsingledescriptivequestion/${sectionDetails.sectionId}`,
        { params: { questionId } }
      );
      setQuestion(questionResponse.data.data);

      const answerResponse = await axios.get(
        `http://13.51.112.252:8000/api/v1/user/getsingledescriptiveanswers`,
        { params: { answerId } }
      );
  ...
  const handleClick = async () => {
    ...
    try {
      await axios.patch(
        `http://13.51.112.252:8000/api/v1/admin/descriptiveMark/${userId}`,
        {
