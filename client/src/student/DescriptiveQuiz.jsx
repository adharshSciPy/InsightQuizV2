...
const DescriptiveQuiz = ({sectionId}) => {
  ...
  const fetchQuestions = async () => {
    if (!sectionId) return;
    try {
      const response = await axios.get(`http://13.51.112.252:8000/api/v1/section/getsectionsbyid/${sectionId}`);
  ...
  const handleSubmitQuiz = async (isDisqualified = false) => {
    try {
        ...
        const response = await axios.post(`http://13.51.112.252:8000/api/v1/user/descriptivequizsubmit/${loggedInUserId}/${sectionId}`, {
